...
void regRLi()
{
	...
	switch(size)
	{
	case 0:	{	_u8 resultb = 0;
				for (i = 0; i < sa; i++) 
				{
					resultb = rCodeB(rCode);
					tempC = FLAG_C;
					SETFLAG_C(resultb & 0x80);
					resultb <<= 1;
					if (tempC) resultb |= 1;
					rCodeB(rCode) = resultb;
				}
				SETFLAG_S(resultb & 0x80);
				SETFLAG_Z(resultb == 0);
				parityB(resultb);
				cycles = 6 + (2*sa);
				break; }
		
	case 1:	{	_u16 resultw = 0;
				for (i = 0; i < sa; i++) 
				{
					resultw = rCodeW(rCode);
					tempC = FLAG_C;
					SETFLAG_C(resultw & 0x8000);
					resultw <<= 1;
					if (tempC) resultw |= 1;
					rCodeW(rCode) = resultw;
				}
				SETFLAG_S(resultw & 0x8000);
				SETFLAG_Z(resultw == 0);
				parityW(resultw);
				cycles = 6 + (2*sa);
				break; }

	case 2:	{	_u32 resultl = 0;
				for (i = 0; i < sa; i++) 
				{
					resultl = rCodeL(rCode);
					tempC = FLAG_C;
					SETFLAG_C(resultl & 0x80000000);
					resultl <<= 1;
					if (tempC) resultl |= 1;
					rCodeL(rCode) = resultl;
				}
				SETFLAG_S(resultl & 0x80000000);
				SETFLAG_Z(resultl == 0);
				cycles = 8 + (2*sa);
				break; }
	}
...
void regRRi()
{
	...
	case 0:	{	_u8 resultb = 0;
				for (i = 0; i < sa; i++) 
				{
					resultb = rCodeB(rCode);
					tempC = FLAG_C;
					SETFLAG_C(resultb & 0x01);
					resultb >>= 1;
					if (tempC) resultb |= 0x80;
					rCodeB(rCode) = resultb;
				}
				SETFLAG_S(resultb & 0x80);
				SETFLAG_Z(resultb == 0);
	...
	case 1:	{	_u16 resultw = 0;
				for (i = 0; i < sa; i++) 
				{
					resultw = rCodeW(rCode);
					tempC = FLAG_C;
					SETFLAG_C(resultw & 0x01);
					resultw >>= 1;
					if (tempC) resultw |= 0x8000;
					rCodeW(rCode) = resultw;
				}
				SETFLAG_S(resultw & 0x8000);
				SETFLAG_Z(resultw == 0);
	...
	case 2:	{	_u32 resultl = 0;
				for (i = 0; i < sa; i++) 
				{
					resultl = rCodeL(rCode);
					tempC = FLAG_C;
					SETFLAG_C(resultl & 0x01);
					resultl >>= 1;
					if (tempC) resultl |= 0x80000000;
					rCodeL(rCode) = resultl;
				}
				SETFLAG_S(resultl & 0x80000000);
				SETFLAG_Z(resultl == 0);
